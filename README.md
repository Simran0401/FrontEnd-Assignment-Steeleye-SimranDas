...
##### I) The `WrappedListComponent` returns each `SingleListItem` by reading from an ordered `items` array in an unordered list format. The `WrappedListComponent` passes the `onClickHandler` down as props to `SingleListItem` component.
