...
##### I. i) The `WrappedListComponent` returns each `SingleListItem` by reading from an ordered `items` array in an unordered list format. 

   ii) The `WrappedListComponent` passes the `onClickHandler` down as props to `SingleListItem` component. 

   iii) The `WrappedListComponent` maintains a state of selection for `SingleListItem` as it passes the `isSelected` down as props. 

   iv) The `WrappedListComponent` passes `key` as `index` to `SingleListItem` in order to maintain the individuality of the component for React.
